...
latest available timestep of the given variant ("nofire" or "fire").

    python3 plot_wind_terrain.py [nofire|fire] [t_seconds]   # default: nofire, latest
...
def latest_plotfile(variant):
    files = sorted(glob.glob(f"plt3d_{variant}?????"),
                    key=lambda f: int(re.search(r"(\d{5})$", f).group(1)))
    if not files:
        sys.exit(f"no plt3d_{variant}##### plotfiles found -- run inputs_{variant} first")
    return files[-1]


def plotfile_at_time(variant, t_seconds):
    step = round(t_seconds / PLOT_CADENCE_S) * int(PLOT_CADENCE_S / 0.25)
    fname = f"plt3d_{variant}{step:05d}"
    if not os.path.isdir(fname):
        sys.exit(f"{fname} not found (requested t={t_seconds}s) -- check available plt3d_{variant}##### files")
    return fname
...
def main():
    variant = sys.argv[1] if len(sys.argv) > 1 else "nofire"
    ...
    for name, (xlo, xhi, ylo, yhi) in {
        "(xhi,ylo) mixed": (DOMAIN_HI - CORNER_SIZE, DOMAIN_HI, 0.0, CORNER_SIZE),
        "(xlo,yhi) mixed": (0.0, CORNER_SIZE, DOMAIN_HI - CORNER_SIZE, DOMAIN_HI),
    }.items():
        ax.add_patch(plt.Rectangle((xlo, ylo), xhi - xlo, yhi - ylo,
                                    fill=False, edgecolor="red", linestyle="--", linewidth=2))
        ax.text(xlo, ylo, name, color="red", fontsize=8, va="bottom", ha="left")

    ax.set_aspect("equal")
    ax.set_xlabel("x [m]"); ax.set_ylabel("y [m]")
    ax.set_title(f"OpenBCRidgeCorner ({variant}): terrain + wind (20 m AGL, lowest model level) at t={t:.0f}s\n{pf}")
